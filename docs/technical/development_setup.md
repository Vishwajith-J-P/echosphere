...
5. `SECRET_KEY` is not an Agora credential. Generate a long random value locally for Flask session/signing protection, for example with a password manager or a cryptographically secure generator, and place it only in the root `.env` file.
...
python -m echosphere.cli create-user --username demo-agent --role agent
python run.py
```
...
- agent authentication: protected demo account with explicit agent/supervisor roles.
